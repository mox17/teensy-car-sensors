...
#define PingConvert(echoTime, conversionFactor) \
    (max((echoTime + conversionFactor / 2) / conversionFactor, \
	     (echoTime ? 1 : 0)))

enum PingTimerReturn
{
    PING_WAITING, // We are waiting for ehco to return, timer running
    PING_ECHO,    // Echo was received

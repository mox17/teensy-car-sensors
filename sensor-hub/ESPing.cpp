...
unsigned int ESPing::ping_median(uint8_t it) {
...
void ESPing::ping_timer(void (*userFunc)(void)) {
    ...
    //Serial.println(_max_time);
	timer_us(ECHO_TIMER_FREQ, userFunc); // Set ping echo timer check every ECHO_TIMER_FREQ uS.
}


PingTimerReturn ESPing::check_timer() {
	if (micros() > _maxEchoTime) { // Outside the timeout limit.
...
void ESPing::timer_us(unsigned int frequency, void (*userFunc)(void)) {
...
void ESPing::timer_stop() { // Disable timer interrupt.
#if defined (__AVR_ATmega32U4__) // Use Timer4 for ATmega32U4 (Teensy/Leonardo).
	TIMSK4 = 0;
#elif defined(__arm__) && defined(TEENSYDUINO)
	itimer.end();
...
unsigned int ESPing::convert_in(unsigned int echoTime) {
	return NewPingConvert(echoTime, US_ROUNDTRIP_IN); // Convert uS to inches.
}

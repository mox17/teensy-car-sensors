...
class Sensorhub:
    ...
    def __init__(self):
        ...
        self.txState = txs.TS_BEGIN
        self.txEscByte = None

        self.rxEscapeFlag = False
        self.rxState = rxs.RS_BEGIN
        self.rxRawData = []
        self.rxChecksum = 0
    ...
    def rxRawByte(self, character):
        b = ord(character)
        if self.rxState == rxs.RS_BEGIN:
            if b == FRAME_START_STOP:
                self.rxState = rxs.RS_DATA
                self.rxNewFrame()
        elif self.rxState == rxs.RS_DATA:
            if b == FRAME_START_STOP:
                self.rxHandleFrame()
                self.rxNewFrame()
                return
    ...
    def txGetNextPacket(self):
        if self.txCurrPacket == None or len(self.txCurrPacket) == 0:
            if not self.txQueue.empty():
                self.txCurrPacket = txAppendChecksum(self.txQueue.get())
                return True
        return False
    ...
    def txDataAvailable(self):
        if (self.txState != txs.TS_BEGIN) or \
            (self.txCurrPacket != None and len(self.txCurrPacket) > 0):
            return True
        else:
            return self.txGetNextPacket()
    
    def txGetByte(self):
        if self.txState == txs.TS_BEGIN:
            if self.txDataAvailable():
                self.txState = txs.TS_DATA
                return [True, FRAME_START_STOP]
            else:
                return [False, None]
    
        elif self.txState == txs.TS_DATA:
            dataAvailable, byte = self.txGetDataByte()
            if dataAvailable == True:
                if (byte == FRAME_START_STOP) or \
                   (byte == FRAME_DATA_ESCAPE):
                    self.txEscByte = byte ^ FRAME_XOR
                    byte = FRAME_DATA_ESCAPE
                    self.txState = txs.TS_ESCAPE
                return [True, byte]
            else:
        ...
        elif self.txState == txs.TS_ESCAPE:
            self.txState = txs.TS_DATA
